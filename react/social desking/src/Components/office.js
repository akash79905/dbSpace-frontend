...
export default class OfficeMap extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.data !== prevState.data) {
      const viewBox = OfficeMap.calculateViewBox(nextProps.data, nextProps.horizontalSize, nextProps.verticalSize);
  ...
  endDrag(event) {
    ...
    if (selectedElement) {
      ...
      if (this.props.onMove && (xPosition !== xPositionBefore || yPosition !== yPositionBefore)) {
        ...
        if (x === xPositionBefore && y === yPositionBefore && visibility === "visible") {
          this.unSelectDesk();
        } else {
  ...
  showEditMode() {
    if (this.props.editMode) {
      return [<rect key="pattern_1" x={0} y={0} width={CELL_QTY * CELL_SIZE} height={CELL_QTY * CELL_SIZE} fill="url(#pattern)" />, <rect key="pattern_2" x={0} y={0} width={CELL_QTY * CELL_SIZE} height={CELL_QTY * CELL_SIZE} fill="url(#pattern_big)" />];
    }
    return undefined;
  }
  ...
  showDesks() {
    return (
      ...
      this.props.data.map((desk) => (
        <use id={desk.id} style={this.props.onMove ? { cursor: "grab" } : {}} key={`key_${desk.id}`} href={`#${this.getDeskId(desk)}`} x={desk.x * CELL_SIZE} y={desk.y * CELL_SIZE} onMouseDown={this.props.onMove ? (event) => this.startDrag(event) : () => {}} onMouseMove={this.props.onMove ? (event) => this.drag(event) : () => {}} onMouseUp={this.props.onMove ? (event) => this.endDrag(event) : () => {}} onClick={this.props.onSelect ? (event) => this.selectDesk(event) : () => {}}>
  ...
  getDeskComponentsTypes(desk) {
    ...
    const definedComponents = ["chair", "chair-na", "drawer", "desk", "keyboard", "mouse", "monitor", "phone", "cpu", "desktop", "laptop"];
  ...
  getDeskComponents(desk) {
    ...
    return deskComponents.map((component) => <use key={`component_${desk.chairDirection}_${component}`} href={`#${component}`} transform={transformMap[desk.chairDirection]} />);
  }
  ...
  render() {
    ...
    return (
      ...
      >
        <defs>
          <g id="chair">
            <rect width="70" height="75" stroke="black" fill="#00FF00" transform="translate(90 90)" strokeWidth="0.7" rx="25" ry="25" />
          ...
          <g id="chair-na">
            <rect width="70" height="75" stroke="black" fill="#FF6347" transform="translate(90 90)" strokeWidth="0.7" rx="25" ry="25" />
            <rect width="60" height="12" stroke="black" fill="#acacac" transform="translate(94 162)" strokeWidth="0.7" rx="15" ry="15" />
            <rect width="10" height="40" style={{ fill: "#acacac", stroke: "black", strokeWidth: "0.7" }} transform="translate(82 105)" rx="3" ry="3" />
            <rect width="10" height="40" style={{ fill: "#acacac", stroke: "black", strokeWidth: "0.7" }} transform="translate(158 105)" rx="3" ry="3" />
            <line x1="95" y1="168" x2="153" y2="168" style={{ stroke: "#5b5b5b", strokeWidth: 0.7 }} rx="3" ry="3" />
          </g>
          <g id="drawer">
